...
* **Добавляет отсутствующие методы** при изменении proto-контракта.
...
```bash
go run ./cmd/upd-stubs --dry-run --verbose --prune
```
...
| `--prune`   | Аннотировать orphaned/stale обработчики (best-effort) |
